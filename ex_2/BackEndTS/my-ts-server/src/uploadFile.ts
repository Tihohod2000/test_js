...
export async function uploadFile(uploadFile: { uuid: string; path: string; createdAt: number; updateAt: number; countDownload: number; }) {
    const key: string = `file:${uploadFile.uuid}`;
    await redis.set(key, JSON.stringify(uploadFile));
    const createdAt: number = uploadFile.createdAt || Date.now();
